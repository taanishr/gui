...
NodeBuilder<Shell> div(Renderer& renderer, float w, float h, float x, float y, simd_float4 color, float cornerRadius) {
...
NodeBuilder<Text> text(Renderer& renderer, const std::string& text, float fontSize, float x, float y) {
    auto& tree = renderer.renderTree;
    auto parent = tree.root.get();
    auto nb = NodeBuilder(tree, parent, std::make_unique<Text>(renderer, x, y, fontSize, simd_float3{1,1,1}));
